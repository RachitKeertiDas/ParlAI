from transformers import BertTokenizer, BertForMaskedLM, AutoModelWithLMHead, AutoTokenizer, BertConfig, AutoConfig, AutoModelForMaskedLM
...
class ClfModel:
      def __init__(self, args):
            ...
            self.rep_model = AutoModelForMaskedLM.from_pretrained("bert-base-uncased").to(self.device)
            ...
            if "svm" in self.model:
                ...
                self.clf = RandomForestClassifier(n_estimators=200,max_samples=0.65)
            else:
      ...
      def encode_bert(self, rep_sen, rep_ids):
            ...
            for id, rep_token in enumerate(rep_tokens):
                ...
                if id >= rep_start_id and id <= rep_end_id:
                    ...
                    mak_um += len(rep_subtoken)
                    rep_actual_subtokens.extend(rep_subtoken)
                else:
                    rep_subtokens.extend(rep_subtoken)
                    rep_actual_subtokens.extend(rep_subtoken)
            rep_subtokens.append("[SEP]")
            rep_actual_subtokens.append("[SEP]")
            rep_input_ids = torch.LongTensor(tokenizer.convert_tokens_to_ids(rep_subtokens)).unsqueeze(0).to(self.device)
            rep_actual_input_ids = torch.LongTensor(tokenizer.convert_tokens_to_ids(rep_actual_subtokens)).unsqueeze(0).to(self.device)
            prediction_scores = model(rep_input_ids)[0]
            prediction_scores = F.softmax(prediction_scores, dim=-1)
            #print('='*10)
            #print(rep_input_ids.shape)
            #print(prediction_scores.shape)
            #print(mak_um)
            scores = []
            for id in range(rep_mask_start_id, rep_mask_end_id):
                subtoken_score = prediction_scores[0, id, rep_input_ids[0][id]].item()
                actual_subtoken_score = prediction_scores[0, id, rep_actual_input_ids[0][id]].item()
                #print(subtoken_score, actual_subtoken_score)
                #print("Printing [Mask] id",rep_input_ids[0][id])
                #print(tokenizer.convert_ids_to_tokens([rep_input_ids[0][id].item()]))
                #scores.append(subtoken_score)
                scores.append(actual_subtoken_score)
...
#            predy = self.clf.predict(real_testx)
            ...
            #print("="*20)
            #print("Features: {}".format(" ".join(best_feats)))
            #acc, info = binary_eval(predy, testy, return_f1=False)
